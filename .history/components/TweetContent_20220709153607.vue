<template>
  <div>
    <div v-for="(data,index) in showTweets" :key="index" class="tweet__show">
      <div class="tweet__show__inner">
        <p class="user__name">{{data.tweetUserName}}</p>
        <div class="like__btn"><img src="../img/heart.png" class="like__img"><span class="likes">{{data.likes}}</span>
        </div>
        <div class="delete__btn" @click="deleteTweet(data.index)"><img src="../img/cross.png" class="delete__img"></div>
        <div class="comment__btn"><img src="../img/detail.png" class="comment__img"></div>
      </div>
      <p class="tweet__content">{{data.tweetContent}}</p>
    </div>
  </div>
</template>
