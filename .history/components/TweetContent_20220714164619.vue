  ...
  <div>
    <div v-for="tweet in showTweets" :key="tweet.id" class="tweet__show">
      <div class="tweet__show__inner">
        ...
        <div class="like__btn" v-if="tweet.user.id == this.$store.state.loginUserId" @click="clickLike(tweet)"><img
            src="../img/heart.png" class="like__img"><span class="likes">{{ tweet.likes.length }}</span>
        </div>
