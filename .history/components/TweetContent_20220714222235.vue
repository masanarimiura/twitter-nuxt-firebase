  ...
  <div>
    <div v-for="tweet in showTweets" :key="tweet.id" class="tweet__show">
      <div class="tweet__show__inner">
        ...
        <div class="like__btn" @click="clickLike(tweet)">
          <img src="../img/heart.png" class="like__img"><span class="likes">{{ tweet.likes.length }}</span>
        </div>
...
export default {
  ...
  methods: {
    async clickLike(tweetData) {
      ...
      const inLikes = tweetData.likes
      const tweetId = tweetData.id
      console.log([inLikes[tweetId].user_id].includes(this.$store.state.loginUserId))
      console.log([inLikes[tweetId].user_id])
      // for (let i = 0; i < inLikes.length; i++) {
      //   console.log([inLikes[i].user_id].includes(this.$store.state.loginUserId))
      // }
