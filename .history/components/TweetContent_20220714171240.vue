  ...
  <div>
    <div v-for="tweet in showTweets" :key="tweet.id" class="tweet__show">
      <div class="tweet__show__inner">
        ...
        <div class="like__btn" v-show="tweet.likes.user_id == $store.state.loginUserId" @click="clickUnlike(tweet)">
        <img src="../img/heart.png" class="like__img"><span class="likes">{{ tweet.likes.length }}</span>
        </div>
        <div class="like__btn" v-show="tweet.likes.user_id != $store.state.loginUserId" @click="clickLike(tweet)">
        <img src="../img/heart.png" class="like__img"><span class="likes">{{ tweet.likes.length }}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    async clickUnlike(tweetData) {
      const sendUnlikeData = {
        user_id: this.$store.state.loginUserId, 
        tweet_id: tweetData.id
      }
      const resData = await this.$axios.post("http://127.0.0.1:8000/api/v1/like", sendUnlikeData);
      // location.reload();
    },
